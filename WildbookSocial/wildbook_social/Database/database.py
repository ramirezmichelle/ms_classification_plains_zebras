...
class Database:
    ...
    # TODO: may consider implementing a safety feature in case relevant prediction turns out not to be true
    def lookThroughRelevantImgsOnly(self, collection, amount):
...
#             item = self.db[collection].find_one({"$and": [{"relevant": True}, {"wild": None}, {"datetaken": {"$gte": self.timeFrameStart}}]})
            print(item)
            if not item:
                break
            ...
            print("CLASSIFIED AS RELEVANT. Decide if Wild(y/n):", end =" ")
            wild = True if input() == 'y' else False 
            
            self._updateItem(collection, item['_id'], {"wild": wild})

            print("Response saved! Relevant and {}.\n".format("Wild" if wild else "Not wild"))
            amount -= 1
